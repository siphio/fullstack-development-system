...
- **Device Support:**
  - Specify which devices must be supported
  - Options: Desktop only / Desktop + Mobile / Desktop + Mobile + Tablet
  - Note any device-specific behaviors (e.g., mobile uses bottom navigation)
  
- **Critical User Flows:**
  - List the primary user journeys that must work flawlessly
  - For each flow: starting point → key steps → successful outcome
  - Prioritize: which flows are MVP-critical vs nice-to-have
...
- **Test User Requirement** (if app has authentication):
  - Standard test user credentials for development/testing
  - Test user should have admin/full access for comprehensive testing
  - Seeding mechanism for test environments
